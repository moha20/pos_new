...
class CartWidget extends StatelessWidget {
                    ...
                    children: [
                      ...
                      if (state.cartItems.isNotEmpty)
                        Chip(
                          label: Text(
                            _getTierBadgeText(
                              state.selectedCustomer?.priceLevel ?? 'retail',
                              isArabic,
                            ),
                            style: TextStyle(
                              fontSize: 10.sp,
                    ...
                    builder: (context, custState) {
                      ...
                      return DropdownButtonFormField<CustomerEntity?>(
                        ...
                        items: [
                          ...
                          ...customers.map(
                            (c) => DropdownMenuItem<CustomerEntity?>(
                              value: c,
                              child: Text(
                                        ...
                                        IconButton(
                                          ...
                                          onPressed: () {
                                            context.read<POSBloc>().add(
                                              POSRemoveProduct(item.product.id),
                                            );
                                          },
                                        ),
                                      ],
  ...
  void _showDiscountDialog(BuildContext context) {
    ...
    final controller = TextEditingController(
      text: context.read<POSBloc>().state.discount.toString(),
    );
    showDialog(
      context: context,
      builder: (context) {
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          title: Text(
            isArabic
                ? 'تعديل سعر الوحدة (مؤقت)'
                : 'Edit Unit Price (Temporary)',
            style: const TextStyle(fontWeight: FontWeight.bold),
          ),
          content: Column(
            mainAxisSize: MainAxisSize.min,
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                ...
                if (val > 0) {
                  context.read<POSBloc>().add(
                    POSUpdateItemPrice(item.product.id, val),
                  );
                }
                Navigator.pop(context);
              },
                ...
                ElevatedButton(
                  onPressed: () {
                    final double finalPaid =
                        double.tryParse(paidController.text) ?? total;
                    if (finalPaid < 0 || (!hasCustomer && finalPaid != total)) {
                      return; // Simple validation check
                    }
                    final double finalRemaining = (total - finalPaid).clamp(
                      0.0,
                      total,
                    );
  ...
  void _showDraftOptionsDialog(
    ...
    final draftSale = SaleEntity(
      id: 'draft',
      invoiceNumber:
          state.invoiceNumber.replaceAll('#', '') +
          (isArabic ? ' (معاينة)' : ' (DRAFT)'),
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              isArabic
                                  ? 'عرض الفاتورة بتنسيق PDF وطباعتها'
                                  : 'View invoice PDF and print it',
                              style: TextStyle(
                                fontSize: 10.sp,
